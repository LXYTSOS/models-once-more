...
def permute_and_flatten(layer, N, A, C, H, W):
    # view函数只能用于contiguous后的tensor上，也就是只能用于内存中连续存储的tensor。如果对tensor调用过
    # transpose, permute等操作的话会使该tensor在内存中变得不再连续，此时就不能再调用view函数。
    # 因此，需要先使用contiguous来返回一个contiguous copy。
